...
class MLRepository:
    ...
    async def get_latest_classification(
        ...
        """Obtém classificacao mais recente de uma entidade."""
        result = await self.session.execute(
            ...
            .where(
                and_(
                    ...
                    MLClassification.entity_id == entity_id,
                )
            )
    ...
    ) -> Sequence[MLClassification]:
        """Obtém classificacoes por tier."""
        query = select(MLClassification).where(
            and_(
                MLClassification.config_id == config_id,
                MLClassification.valid_until > datetime.utcnow(),
            )
        )
        if entity_type:
            query = query.where(MLClassification.entity_type == entity_type)
        if tier:
            query = query.where(MLClassification.tier == tier)
    ...
    def _apply_classification_filters(
        ...
        query = query.where(MLClassification.config_id == config_id)
        if entity_type:
            query = query.where(MLClassification.entity_type == entity_type)
        if entity_id:
            query = query.where(MLClassification.entity_id == entity_id)
        if tier:
            query = query.where(MLClassification.tier == tier)
        if min_confidence is not None:
            query = query.where(MLClassification.confidence_score >= min_confidence)
        if start_date:
            query = query.where(MLClassification.classified_at >= start_date)
        if end_date:
            query = query.where(MLClassification.classified_at <= end_date)
        return query
    ...
    async def get_classifications(
        ...
        """Obtém classificacoes com filtros e paginacao."""
        query = select(MLClassification)
        ...
        query = query.order_by(desc(MLClassification.classified_at))
        if offset:
    ...
    async def count_classifications(
        ...
        query = select(func.count(MLClassification.id))
        query = self._apply_classification_filters(
            query,
            config_id=config_id,
            entity_type=entity_type,
    ...
    async def get_classification_counts_by_tier(
        ...
        query = select(
            MLClassification.tier,
            func.count(MLClassification.id),
        )
        query = self._apply_classification_filters(
            query,
            config_id=config_id,
        ...
        query = query.group_by(MLClassification.tier)
        result = await self.session.execute(query)
        counts = {}
    ...
    async def feature_exists(
        ...
        """Verifica se features ja existem para data e entidade."""
        query = select(func.count(MLFeature.id)).where(
            and_(
                MLFeature.config_id == config_id,
                MLFeature.entity_type == entity_type,
    ...
    async def get_features(
        ...
        """Obtém features de uma entidade."""
        query = select(MLFeature).where(
            and_(
                MLFeature.config_id == config_id,
                MLFeature.entity_type == entity_type,
                MLFeature.entity_id == entity_id,
        ...
        if window_days is not None:
            query = query.where(MLFeature.window_days == window_days)
        if start_date:
            query = query.where(MLFeature.feature_date >= start_date)
        if end_date:
            query = query.where(MLFeature.feature_date <= end_date)

        query = query.order_by(desc(MLFeature.feature_date)).limit(limit)
        result = await self.session.execute(query)
        return result.scalars().all()
