...
from projects.ml.services.feature_engineering import CampaignFeatures, FeatureEngineer
from projects.ml.db.models import CampaignTier
...
def get_classifier(model_path: Optional[str] = None) -> CampaignClassifier:
    """
    Factory function to create CampaignClassifier instances.
    ...
    Returns:
        New CampaignClassifier instance
    """
    return CampaignClassifier(model_path=model_path)
...
class _LazyClassifier:
    """Lazy wrapper that creates classifier on first access with deprecation warning."""

    _instance: Optional[CampaignClassifier] = None

    def __getattr__(self, name):
        import warnings
        warnings.warn(
            "Global 'classifier' is deprecated. Use get_classifier() instead.",
            DeprecationWarning,
            stacklevel=2
        )
        if self._instance is None:
            self._instance = CampaignClassifier()
        return getattr(self._instance, name)


# Deprecated global instance - use get_classifier() instead
