...
from projects.agent.memory.checkpointer import AgentCheckpointer, get_agent_checkpointer
...
def get_agent():
    ...
    if should_use_multiagent():
        from projects.agent.orchestrator import get_orchestrator
...
class TrafficAgentService:
    ...
    async def _chat_multiagent(
        ...
        logger.info("Chat multi-agente: thread={thread_id}")
    ...
    async def chat_multi_agent(
        ...
        try:
            ...
            logger.info("Processando mensagem multi-agente: thread={thread_id}")
            ...
            if result.get("error"):
                logger.warning("Orchestrator retornou erro: {result.get('error')}")
                return {
                    "success": False,
                    "thread_id": thread_id,
            ...
            if not synthesized_response:
                ...
                logger.warning("Resposta vazia do orchestrator: thread={thread_id}")
            ...
            logger.error("Erro no chat_multi_agent", error=str(e), exc_info=True)
            return {
                "success": False,
                "thread_id": thread_id,
                "error": str(e),
    ...
    async def stream_chat_multi_agent(
        ...
        try:
            ...
            logger.error("Erro no stream_chat_multi_agent", error=str(e), exc_info=True)
            yield {
                "type": "error",
                "error": str(e),
                "thread_id": thread_id,
    ...
    async def stream_chat(
        ...
        try:
            ...
            logger.error("Erro no stream", error=str(e))
            yield {
                "type": "error",
                "error": str(e),
                "thread_id": thread_id,
    ...
    async def clear_conversation(self, thread_id: str) -> bool:
        ...
        try:
            ...
            logger.info("Conversa {thread_id} limpa")
            return True
