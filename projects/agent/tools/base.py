...
def agent_tool(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            logger.debug("Executando tool", tool_name=tool_name, args=args, kwargs=kwargs)

            try:
                result = await func(*args, **kwargs)
                logger.debug("Tool concluída", tool_name=tool_name, result_type=type(result).__name__)
                return result
            except Exception as e:
                logger.error("Erro na tool", tool_name=tool_name, error=str(e))
